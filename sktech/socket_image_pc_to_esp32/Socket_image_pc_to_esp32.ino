...
void loop()
{
  ...
  //Serial.print("displaying image on tft , size is ");
  //Serial.println(sizeof(imagebuf));
...
void renderJPEG(int xpos, int ypos)
{
  ...
  uint16_t mcu_h = JpegDec.MCUHeight; // 16
  uint32_t max_x = JpegDec.width;     // 240
  uint32_t max_y = JpegDec.height;    // 135

  Serial.println("    ");
  Serial.println("");
  Serial.println("  ender info  ");
...
void getImage(WiFiClient client)
{
  ...
  {
    last_image_size = client.available();
    //imagebuf={ NULL} ;
